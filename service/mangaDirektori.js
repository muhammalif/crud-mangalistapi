...
async function create(mangaData, fileMangaData) {
  try {
    const result = await db.query(
      ...
      VALUES 
      ('${mangaData.judul}', '${mangaData.penulis}', '${mangaData.penerbit}', '${mangaData.tanggal_rilis}', ${mangaData.rating}, ${mangaData.jumlah_volume}, 'http://localhost:3001/uploads/${fileMangaData.originalname}')`
    );
  ...
  try {
    ...
    if (row.length > 0) {
      ...
      if (fileMangaData) {
        ...
        url_baca = `http://localhost:3001/uploads/${fileMangaData.originalname}`;
      }
      const result = await db.query(
        `UPDATE manga 
